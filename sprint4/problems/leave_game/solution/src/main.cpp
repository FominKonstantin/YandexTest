...
int main(int argc, const char* argv[]) {
  ...
  try {
    ...
    // Инициализируем RecordManager (подключение к PostgreSQL)
    std::unique_ptr<records::RecordManager> record_manager;
    try {
      const char* db_url_env = std::getenv("GAME_DB_URL");
      if (!db_url_env || std::string(db_url_env).empty()) {
        std::cerr << "Error: GAME_DB_URL environment variable is not set"
                  << std::endl;
        return EXIT_FAILURE;
      }

      std::string db_url(db_url_env);
      record_manager = std::make_unique<records::RecordManager>(db_url);
      record_manager->InitializeTable();
    ...
    // Создаем RequestHandler с передачей RecordManager
    auto handler = std::make_shared<http_handler::RequestHandler>(
        game, players, state_manager, args.state_file, args.www_root,
        args.config_file, args.randomize_spawn, api_strand, *record_manager);
